...
def load_default_answer(exercise_number):
    try:
        with open(f"{exercise_number}.md", "r") as file:
            ...
            # Remove <think>...</think> block from the default answer
            clean_default_answer = re.sub(r'<think>.*?</think>', '', default_answer, flags=re.DOTALL)
            return clean_default_answer
...
# CSS for styling answer areas with borders and color
st.markdown("""
    <style>
        .answer-area {
...
async def display_exercise(exercise_number, exercise_question, default_answer):
    ...
    # Display default answer with styled area
    st.subheader("AI's Default Solution")
    st.markdown(f'<div class="answer-area">{default_answer}</div>', unsafe_allow_html=True)
    ...
    if st.button(f"Regenerate Answer for Exercise {exercise_number}"):
        ...
        st.markdown(f'<div class="answer-area">{response}</div>', unsafe_allow_html=True)
